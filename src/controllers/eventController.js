...
const postEvent = async (req, res, next) => {
  try {
    ...
    res.status(200).json({message:"event created successfully👍", event:updatedEvent});
  } catch (err) {
    console.error('Error saving/updating event:', err);
...
// Function to get passed events
const getPassedEvents = async (req, res, next) => {
  try {
    const passedEvents = await Event.find({ date: { $lt: new Date() } }).sort({ date: -1 });
    res.status(200).json(passedEvents);
  } catch (err) {
    console.error('Error retrieving passed events:', err);
    next(err);
  }
};

module.exports = { postEvent, deleteEvent, getAllEvents, getUpcomingEvents, getPassedEvents }; 
