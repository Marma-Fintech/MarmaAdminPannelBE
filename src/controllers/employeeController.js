...
const signup = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Failed to signup', error: error.message || 'Internal Server Error' });
  }
};
...
const login = async (req, res, next) => {
  ...
  res.status(200).json({ message: 'Login successful', token });
    ...
    res.status(500).json({ message: 'Failed to login', error: error.message || 'Internal Server Error' });
  }
};
...
module.exports = {signup, login, signOut, protect};
